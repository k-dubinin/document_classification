...
В UI есть вкладки:
- **Автоматическая классификация**: пакетная обработка директории документов (копирование по папкам классов, лог процесса, CSV-отчёт).
...
- выходная директория **не очищается** автоматически после проведенной классификаци;
- ошибки в классификации отдельных файлов не останавливают общий процесс.

После завершения:
- выводится итоговая сводка (всего / успешно / требуют проверки / ошибок);
- показывается список файлов для ручной проверки;
