"""
UI для системы классификации документов (локально).
...
with tab_auto:
    ...
    with col_b:
        ...
        st.markdown(
            ...
            "Ошибки (битый файл, пустой текст, OCR не помог, неподдерживаемый формат) не останавливают процесс."
        )
...
with tab_about:
    ...
    st.markdown(
        ...
        "- Все артефакты (модель, метрики, confusion matrix) сохраняются в `models/` (или выбранную директорию `--out`)."
    )
